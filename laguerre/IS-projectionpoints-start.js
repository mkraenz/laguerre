...
function createParameterMidpoints() {
	...
	var createParameterMidpointsSubroutine = function(planeIndex1, planeIndex2,
		...
		ggbApplet.evalCommand('r_{' + regionIndex + '} = Slider[0.01, 0.99, 0.01]'); // min, max, increment step
		ggbApplet.evalCommand('SetValue[r_{' + regionIndex + '}, 0.5]');
		ggbApplet.evalCommand('Midpoint_{'
				+ regionIndex + '} = Point[midpointRay_{' + regionIndex + '}, r_{' + regionIndex + '}]');
	}
